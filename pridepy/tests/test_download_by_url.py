...
class TestDownloadFilesByUrl(TestCase):
    ...
    def test_threads_use_public_http_file(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            target = os.path.join(tmp_dir, "sample.raw")

            def fake_http_file(_url, target_path, position=0, download_threads=1):
                assert download_threads == 4
                assert position == 0
                _touch_valid(target_path)

            with patch.object(
                by_url.transport, "download_http_file", side_effect=fake_http_file
            ) as mock_http_file:
                Files.download_files_by_url(
                    urls=["https://example.org/sample.raw"],
                    output_folder=tmp_dir,
            ...
            mock_http_file.assert_called_once()
            assert os.path.exists(target)

    def test_public_http_file_replaces_final_after_part_completes(self):
        with tempfile.TemporaryDirectory() as tmp_dir:
            target = os.path.join(tmp_dir, "sample.raw")
            part = target + ".part"
            ...
            def fake_multipart(_url, target_path, **_kwargs):
                assert target_path == part
            ...
            with patch.object(
                transport, "_multipart_download", side_effect=fake_multipart
            ):
                transport.download_http_file(
                    "https://example.org/sample.raw",
                    target,
                    download_threads=4,
                    multipart_min_size_bytes=1,
                )
    ...
    def test_multipart_existing_full_size_file_is_not_trusted(self):
        class FakeHeadResponse:
            headers = {"content-length": str(20 * 1024 * 1024), "accept-ranges": "bytes"}
